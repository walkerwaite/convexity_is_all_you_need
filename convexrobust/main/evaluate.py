...
def evaluate_models(
    ...
    for (signal, target) in datamodule.eval_iterator(do_tqdm=True):
        for (name, model) in models_to_eval.items():
            t0 = time.time()
            # Always use certify for both classes if available
            if hasattr(model, "certify") and not model.external_certification:
                pred, certificate = model.certify(signal, target)
                debug_vars = { 'cert_time': time.time() - t0 }
                # Set the certificate to zero if the prediction is incorrect
                if pred != target and certificate is not None:
