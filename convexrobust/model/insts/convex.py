...
from convexrobust.model.modules import ConvexMLP

class ConvexMnist(ConvexCertifiable):
    def __init__(self, nonlin=nn.ReLU, mlp_params={}, generic_normalize=False, **kwargs):
        default_params = {
        ...
        super().__init__(
            convex_model=ConvexMLP(in_n=784, **combined_params, nonlin=nonlin,),
            **kwargs
        )
...
class ConvexFashionMnist(ConvexCertifiable):
    def __init__(self, nonlin=nn.ReLU, convnet_params={}, **kwargs):
        default_params = {
            'feature_n': 4, 'depth': 2,
        ...
        super().__init__(
            convex_model=ConvexConvNet(image_size=28, channel_n=1, **combined_params, nonlin=nonlin), # Pass nonlin to BaseCertifiable
            **kwargs
        )
...
class ConvexMalimg(ConvexCertifiable):
    def __init__(self, nonlin=nn.ReLU, mlp_params={}, **kwargs):
        default_params = {
            'feature_ns': [200, 50], 'skip_connections': True, 'batchnorms': True
        }
        combined_params = {**default_params, **mlp_params}  # potentially overwrite defaults
        ...
        super().__init__(
            convex_model=ConvexMLP(in_n=512 * 512, **combined_params, nonlin=nonlin),
            **kwargs
        )
...
class ConvexCifar(ConvexCertifiable):
    def __init__(self, nonlin=nn.ReLU, convnet_params={}, apply_feature_map=True, **kwargs):
        default_params = {
        ...
        super().__init__(
            convex_model=ConvexConvNet(image_size=32, channel_n=channel_n, **combined_params, nonlin=nonlin),
            **kwargs
        )
...
class ConvexSimple(ConvexCertifiable):
    def __init__(self, nonlin=nn.ReLU, **kwargs):
        super().__init__(
            convex_model=ConvexMLP(
                in_n=4, features_n=[50], batchnorms=True, skip_connections=True, nonlin=nonlin),
            **kwargs
        )
