...
from convexrobust.model.modules import ConvexModule, beta  # Import beta from modules.py
...
class ConvexCertifiable(BaseCertifiable):
    ...
    def certify(self, x: Tensor, _) -> tuple[Tensor, Certificate]:
        ...
        # Certified radii for class 1 (sensitive class)
        if pred < 0:
        ...
        # Certified radii for class 2 (non-sensitive class)
        if pred >= 0:
            certificate = Certificate({
                Norm.L1: (math.sqrt(jac.norm(float('inf')).item()**2 - 2 * beta * margin) - jac.norm(float('inf')).item()) / beta,
                Norm.L2: (math.sqrt(jac.norm(2).item()**2 - 2 * beta * margin) - jac.norm(2).item()) / beta,
            Norm.LInf: (math.sqrt(jac.norm(1).item()**2 - 2 * beta * margin) - jac.norm(1).item()) / beta
            })

        predicted_class = int((pred >= 0).item())
        return torch.tensor([predicted_class]), certificate
